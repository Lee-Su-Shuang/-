...
def main(args=None):
  ...
  # 更改fc输出为2，即x、y坐标值
  model = models.mobilenet_v3_small(weights='IMAGENET1K_V1') # 加载预训练的MobileNetV3 Small模型
  model.classifier[3] = torch.nn.Linear(in_features=1024, out_features=2)  # 修改最后一层为2输出，分别对应x和y坐标
