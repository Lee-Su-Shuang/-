...
# --------- 4. 加载 MobileNetV3 Small 模型 ---------
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

model = models.mobilenet_v3_small(weights=MobileNet_V3_Small_Weights.IMAGENET1K_V1)
model.classifier[3] = torch.nn.Linear(in_features=1024, out_features=2)  # 输出两个数 (x, y)
...
with torch.no_grad():
    for images, labels in test_loader:
        ...
        # 注意：**因为训练时label是归一化[-1,1]，所以这里输出是[-1,1]**
        # 要反归一化：pred_y_pixel = (pred_y_normalized * 112) + 112
        y_pred_batch = outputs[:, 1].cpu().numpy() * 112 + 112
